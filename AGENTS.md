...
The repository also contains `windows/`, a separate .NET 10 WPF Clicky prototype. It captures the foreground application before activating its typed-question field, then uses the active-window title and screenshot to guide one visible action at a time in any desktop app. Adobe apps, Visual Studio, and Rive are examples, not explicit integrations. It supports the existing Worker `/chat` route plus direct Anthropic and OpenAI modes, parses a terminal `POINT` directive, and displays a click-through non-activating cue. The Windows prototype has no microphone, transcription, TTS/audio, persisted non-secret settings, installer, or live API E2E verification yet.
...
- **Element Pointing**: The tutor must end with `[POINT:x,y:label]`, `[POINT:x,y:label:screenN]`, or `[POINT:none]`. The parser strips the directive, the mapper converts capture pixels to physical desktop coordinates, and a topmost transparent overlay displays the target without activation or hit testing.
- **Settings and secrets**: Provider, Worker URL, model IDs, capture optimization, and other non-secret settings are memory-only. Direct Anthropic/OpenAI keys persist in Windows Credential Manager under `Clicky/ProviderApiKey/v1/{provider}` and are never redisplayed; legacy Knobnote targets migrate on first use. Capture-only-during-request, companion exclusion, and large-capture optimization are on; pointer, local retention, and all-displays are off. GDI capture does not explicitly remove visible Clicky overlap.
...
| File | Purpose |
|------|---------|
| `windows/src/Clicky.Windows/App.xaml.cs` | Windows startup composition, tray/window shutdown, Worker client, capture service, and overlay lifecycle. |
...
| `windows/src/Clicky.Windows/ViewModels/CompanionViewModel.cs` | Typed interaction orchestration, cancellation, response state, and point cue presentation. |
